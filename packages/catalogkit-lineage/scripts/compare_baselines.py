...
from catalogkit.lineage import trace_downstream
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare catalogkit-lineage against baseline lineage approaches."
    )
...
def compare_sqlglot() -> str:
    ...
    catalogkit_downstream = trace_downstream(
        JAFFLE_MANIFEST,
        dialect="postgres",
    ...
    catalogkit_formatted = [
        item.removeprefix("column:") for item in catalogkit_downstream
    ]
    ...
    lines = [
        ...
        f"- `catalogkit-lineage` downstream traversal returned: `{catalogkit_formatted}`.",
        "- Differentiation: `catalogkit-lineage` owns project loading, reverse traversal, and stable `column:` IDs instead of leaving that glue to the caller.",
    ]
    return "\n".join(lines)
...
def compare_dbt_manifest() -> str:
    ...
    lines = [
        ...
        f"- `catalogkit-lineage` resolves the column-level blast radius for `raw_payments.amount`: `{formatted_columns}`.",
        "- Differentiation: dbt manifest lineage is model-level DAG metadata; `catalogkit-lineage` answers the pre-merge column impact question directly.",
    ]
    return "\n".join(lines)
...
def compare_canva() -> str:
    ...
    lines = [
        ...
        "- `catalogkit-lineage` answers the same pre-merge downstream question in one headless command against a manifest or SQL folder, and emits a mergeable `CatalogArtifact` instead of tool-specific output files.",
        ...
        "- Differentiation: Canva gives recursive column lineage and Mermaid output, but it does not replace CatalogKit's single-command impact traversal, shared artifact contract, or non-dbt SQL-folder path.",
    ]
    return "\n".join(lines)
