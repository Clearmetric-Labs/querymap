...
from clearmetric.core import CatalogArtifact, Edge, Evidence, Warning, merge
...
def build_query_map_from_parsed(parsed: ParsedStatement) -> QueryMap:
    """Build the public clearmetric-core artifact for one parsed SQL statement."""
...
def build_catalog_artifact_from_parsed(parsed: ParsedStatement) -> CatalogArtifact:
    """Build the shared catalog artifact used for ClearMetric Core composition."""
...
def _relation_edge_from_catalog_edge(edge: Edge) -> RelationEdge:
    if edge.kind not in _QUERYMAP_EDGE_KINDS:
        raise QueryMapContractError(
            f"clearmetric-core cannot emit unsupported edge kind {edge.kind!r} in its public contract."
        )
...
def _warning_entry_from_warning(warning: Warning) -> WarningEntry:
    if warning.code not in _QUERYMAP_WARNING_CODES:
        raise QueryMapContractError(
            f"clearmetric-core cannot emit unsupported warning code {warning.code!r} in its public contract."
        )
    return WarningEntry(
